...
while test $x -lt 40;do
  curl  -s --digest --user homer:elmo "http://${host}:${port}/read_bytes?id=${id}&n=555" 2>/dev/null | wc -l  || break
  x=$(($x + 1))
done
...
echo "Compressed stream download, this can take as long as a few minutes..."
id=$(curl -f -s --digest --user homer:elmo "http://${host}:${port}/new_session" | sed -e "s/.*//")
echo $id
...
id=$(curl -f -s -k --digest --user homer:elmo "https://${host}:${psec}/new_session" | sed -e "s/.*//")
echo $id
curl -f -s -k --digest --user homer:elmo "https://${host}:${psec}/execute_query?id=${id}&query=build(%3Cx%3Adouble%3E%5Bi%3D1%3A1000000%2C10000%2C0%5D%2Ci)&save=(double)&stream=1"

...
echo "session: $s"
curl -s -k "http://${host}:${port}/execute_query?id=${s}&query=list('functions')&save=dcsv&stream=2"
echo
...
s=`curl -s -k http://${host}:${port}/new_session`
echo "session: $s"
curl -s -k "http://${host}:${port}/execute_query?id=${s}&query=list('functions')&save=dcsv&stream=2&compression=9"
echo
curl -s -k "http://${host}:${port}/read_bytes?id=${s}&n=0" >/tmp/z.gz
# Decompress
gunzip -c /tmp/z.gz
